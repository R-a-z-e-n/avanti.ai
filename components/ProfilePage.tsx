...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, onLogout, onUnlock, onTierChange }) => {
  const allLanguages = [
      ...
      <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-10">
        <div className="relative">
          ...
          {user.membershipTier === 'PRO' && (
            <div className="absolute -top-2 -right-2 bg-amber-400 text-white w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg border-2 border-white">
        ...
        <div className="flex-1 text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-4 mb-2">
            ...
            <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${user.membershipTier === 'PRO' ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-500'}`}>
              {user.membershipTier}
            </span>
          </div>
          <p className="text-gray-400 font-medium mb-6">{user.email}</p>
          <div className="flex flex-wrap justify-center md:justify-start gap-4">
            <div className="bg-indigo-50 px-4 py-2 rounded-2xl min-w-[100px]">
            ...
            <div className="bg-emerald-50 px-4 py-2 rounded-2xl min-w-[100px]">
            ...
            <div className="bg-rose-50 px-4 py-2 rounded-2xl min-w-[100px]">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-3 py-1 rounded-full uppercase tracking-widest">
              {user.membershipTier === 'PRO' ? 'FREE FOR PRO' : 'Cost: 100 🪙'}
            </span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {availableToUnlock.map(lang => (
              <button 
                ...
                disabled={user.membershipTier !== 'PRO' && user.tokens < 100}
